...
func (h *IncomeHandler) InsertIncome(ctx *fiber.Ctx) error {
	c := ctx.UserContext()

	request := new(request.Income)
	...
	result, err := h.s.Income.Insert(c, request)
	if err != nil {
...
func (h *IncomeHandler) UpdateIncome(ctx *fiber.Ctx) error {
	id := ctx.Params("id")
	request := new(request.UpdateIncome)
	...
	result, err := h.s.Income.Update(ctx.Context(), id, request)
	if err != nil {
		if errors.Is(err, service.BaseErrInsert) {
			return ctx.Status(fiber.StatusBadRequest).JSON(utils.ApiResponse{
				Success:   false,
...
func (h *IncomeHandler) GetIncomes(ctx *fiber.Ctx) error {
	year := ctx.QueryInt("year", 0)
	month := ctx.QueryInt("month", 0)

	c := ctx.UserContext()

	result, err := h.s.Income.GetIncomes(c, year, month)
	if err != nil {
